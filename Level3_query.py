...
def get_accident_analysis(postcode=None, light=None, atmo=None, road=None,
    ...
    if road:
        road_join = """
            JOIN Surface_Cond_Seq scs_f ON a.ACCIDENT_NO = scs_f.ACCIDENT_NO AND scs_f.SURFACE_COND_SEQ = 1
            JOIN Road_Surface_Cond rsc_f ON scs_f.SURFACE_COND = rsc_f.SURFACE_COND
        """
        ph = ",".join("?" * len(road))
